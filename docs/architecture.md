...
Passwords use salted PBKDF2 derivation. Only token hashes are persisted, tokens expire, logout revokes them, and repeated failures temporarily lock the account. API access resolves one active workspace membership and enforces viewer, editor, admin or owner permissions. Admins and owners can create accounts and activate, deactivate or remove workspace memberships; deactivation revokes the user's existing sessions, owner changes remain owner-only and the last active owner is protected. Workspace IDs participate in flow/version primary keys and provider-call uniqueness constraints, so tenants may safely reuse their own identifiers. With authentication disabled, the same code uses the `default` offline workspace.

### Operations

Terminal sessions have configurable retention. Metrics are computed from persisted sessions and their trace timestamps, including volume, state, intent, channel, outcomes, wrap-up codes, completion rate and duration. A queue node pauses the engine in a traceable `queued` state until an on-queue agent claims it. Persistent presence and routing status distinguish availability from active interaction work. With authentication enabled, agent operations resolve the agent name from the signed-in account and reject impersonation; manual identity remains an offline-mode convenience. An agent-assisted session reaches `wrap_up` before completion, where a bounded wrap-up code and optional notes finish after-call work. Sensitive management operations append workspace-scoped audit events; request IDs and structured completion logs make API failures correlatable without logging request bodies.
