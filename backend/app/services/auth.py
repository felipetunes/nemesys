...
class AuthService:
    ...
    def login(
        ...
        if not verify_password(password, user.password_hash):
            ...
            self.db.commit()
            raise AuthError("Invalid email or password")
        if not self._memberships(user.id, active_only=True):
            raise AuthError("Account has no active workspace memberships")
        user.failed_login_attempts = 0
    ...
    def resolve_token(self, token: str, requested_workspace_id: str | None) -> AuthenticatedUser | None:
        ...
        if user is None:
            return None
        memberships = self._memberships(user.id, active_only=True)
        membership = next(
    ...
    def workspaces_for_user(self, user_id: str) -> list[WorkspaceInfo]:
        memberships = self._memberships(user_id, active_only=True)
    ...
    def add_workspace_member(self, workspace_id: str, email: str, role: WorkspaceRole) -> WorkspaceMember:
        ...
        existing = self.db.get(WorkspaceMembershipRow, key)
        if existing is not None and existing.active:
            raise AuthError("User is already a member of this workspace")
        if existing is None:
            self.db.add(WorkspaceMembershipRow(user_id=user.id, workspace_id=workspace_id, role=role, active=True))
        else:
            existing.role = role
        ...
        return self._workspace_member(user, role, active=True)
    ...
    def create_workspace_user(
        ...
        user = self.db.scalar(select(UserRow).where(UserRow.email == normalized_email))
        if user is not None:
            return self.add_workspace_member(workspace_id, normalized_email, role)
        user = UserRow(id=str(uuid4()), email=normalized_email, password_hash=hash_password(password))
        self.db.add(user)
        self.db.add(
            WorkspaceMembershipRow(
                user_id=user.id,
    ...
    def set_workspace_member_active(
        ...
        if membership is None or user is None:
            raise AuthError("Workspace member not found")
        if membership.active and not active and membership.role == "owner":
            self._require_another_owner(workspace_id, user_id)
        membership.active = active
        if not active:
            self.db.execute(delete(AuthSessionRow).where(AuthSessionRow.user_id == user_id))
        self.db.commit()
        return self._workspace_member(user, cast(WorkspaceRole, membership.role), active=active)
    ...
    @staticmethod
    def _workspace_member(user: UserRow, role: WorkspaceRole, *, active: bool) -> WorkspaceMember:
    ...
    def _memberships(self, user_id: str, *, active_only: bool = False) -> list[WorkspaceMembershipRow]:
        query = select(WorkspaceMembershipRow).where(WorkspaceMembershipRow.user_id == user_id)
        if active_only:
            query = query.where(WorkspaceMembershipRow.active.is_(True))
        return list(
            self.db.scalars(
                query.order_by(WorkspaceMembershipRow.workspace_id)
            ).all()
        )
